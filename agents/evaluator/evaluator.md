...
### Step 6：端到端 SSOT 验证（**子任务级默认跳过，仅任务收口跑一次**）

**重要更新**（2026-05-21）：端到端验证（启服务 + 真 LLM + 全链路 SSOT）成本太高（单次 5-10 分钟 + 大量 token），不再每个 step 强制独立跑。规则改为：

| 阶段 | 端到端策略 |
|------|-----------|
| 子任务级 evaluator | **默认跳过**真 LLM 端到端；仅复跑单测 / 结构 grep / 静态断言（DB schema、字段结构、契约对齐）。如有真启服务的低成本断言（如 schema 验证、列表 API 不调 LLM）可保留 |
| 任务收口（Step 5 / 最后一步） | **跑一次完整端到端**——主会话亲自跑或专门指派一个 evaluator，覆盖所有 step 的核心 SSOT |

**子任务级 evaluator 的最小复跑集**（必跑）：
1. 命令复跑：`mvn compile` + 该 step 涉及的单测（不要 `mvn test` 全跑——只跑该 step 影响的测试类）
2. 代码结构 grep（按契约 §代码结构验证）
3. 数据/字段比对（如契约声明且不需要启 LLM）
4. 契约边界复核（已冻结边界 + 越界项）
...
**任务收口端到端跑哪些**（参考各 plan §端到端 SSOT 验证段，挑核心 5-10 条）：
- DB 真值（sessions/messages/game_runs 行数 + 关键字段）
- API 真调用（curl + jq）
- 跨 step 联动（如 Step 4 的 clone 触发 4a 的 endpoint，影响 4b 的 UI 状态）

**与 coder 报告对比**：
- coder 报告"单测/结构/字段过了"，evaluator 复跑同条目不一致 → ❌ 打回
- coder 跳过单测 / 结构验证 → ⚠️ 契约问题
- coder 跳过端到端：**子任务级允许**；但任务收口阶段必须有人跑过

### Step 7：耗时监控（必备）

执行验收命令时记录耗时：
- 单条命令 > 60 秒：在报告里标 ⚠️
- 单条命令 > 5 分钟：考虑是否撞到外部依赖（LLM、网络），不是 step 本身的问题
- 总耗时 > 15 分钟：报告主会话，考虑拆分或简化

**不要**为了"严格"硬等：LLM 一调就是几分钟，等十次它一次也不准——靠少跑、跑准、跑值得跑的。
