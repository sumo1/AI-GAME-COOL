...
3. [x] **Step 3：服务编排** — SessionService + GameChatController 改造 + 老服务 @Deprecated（mvn test 12/12 + 9/9 端到端 SSOT 通过 @ 2026-05-21，commit `160d93a`）
4. [x] **Step 4a（并行）：后端 API** — 新增 `/api/sessions/*` 11 端点；`/api/game/storage/list` 改读 game_runs（mvn test 8/8 + 端到端 10/10 通过 @ 2026-05-21，commit `3b75fab`）
5. [x] **Step 4b（并行）：前端 UI** — `ServerGameHistory` 改读新接口、新增 `SessionHistory` 抽屉、收藏按钮（npx tsc + npm build + browser-harness 真验证通过 @ 2026-05-21，commit `3b75fab`）
6. [x] **Step 5：清理与文档** — conventions §12/§13、CLAUDE.md、README、knowledge/pitfalls 上浮、轻量端到端 9/9 SSOT @ 2026-05-21
...
| Step 5 收口拆分 | 2026-05-21 | "贪吃蛇可玩性自动验证"超出存储任务范围，独立成新任务 `260521-playability-oracle`。Step 5 只做：轻量端到端（无 LLM 也能验的 API SSOT）+ 文档同步 |
| Step 5 端到端 LLM 失败但通过 | 2026-05-21 | DashScope free tier 当日耗尽（403 AllocationQuota.FreeTierOnly）。**反向证明**了 SessionService 容错路径正确：DB 仍写入 sessions+messages、game_runs=0、assistant content 含 [失败] 前缀。9/9 SSOT 断言全部通过（覆盖 list/messages/games/favorites/clone/delete/老兼容接口） |
